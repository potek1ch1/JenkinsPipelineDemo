    ...
    stages {
        ...
        stage('Test') {
            steps {
                echo 'Testing'
                script {
                    def url = 'https://test-env-jenkins-potekichi.s3.ap-northeast-1.amazonaws.com/index.html'
            ...
            steps {
                echo 'Releasing'
                 withCredentials([[
                    ...
                    accessKeyVariable: 'AWS_ACCESS_KEY_ID',
                    secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]){
                        sh(script: 'aws s3 cp /var/lib/jenkins/workspace/JenkinsPipeline/index.html s3://prod-env-jenkins-potekichi/')
                }
            } 
        }
    }
